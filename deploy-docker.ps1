# Wedding Gift List - Docker-based Tag Deployment Script (PowerShell)
# Creates and pushes version tags to trigger Docker container deployments on AWS EC2
...
Write-Host "*** Wedding Gift List - Docker Deployment ***" -ForegroundColor $Blue
Write-Host "============================================="
...
Write-Host "1) Frontend only (GitHub Pages)"
Write-Host "2) Backend only (Docker Container on EC2)"
...
switch ($Choice) {
    ...
    "2" {
        ...
        $Description = "Backend Docker deployment v$Version"
        $DeployType = "Backend (Docker)"
    }
    "3" {
        $Tag = "v$Version"
        $Description = "Full application deployment v$Version"
        $DeployType = "Full application (Frontend + Docker Backend)"
    }
    default {
...
switch ($Choice) {
    ...
    "2" {
        Write-Host "   - Backend will deploy as Docker container on AWS EC2"
        Write-Host "   - Expected deploy time: ~5-8 minutes"
        Write-Host "   - API will be available at: http://18.219.60.144/api/gifts"
    }
    "3" {
        Write-Host "   - Frontend will deploy to: https://lucasmedeiros.github.io/wedding-gift-list"
        Write-Host "   - Backend will deploy as Docker container on AWS EC2"
        Write-Host "   - Expected total deploy time: ~8-10 minutes"
    }
}

Write-Host ""
Write-Host ">> Docker Deployment Features:" -ForegroundColor $Blue
Write-Host "   - Containerized application for better isolation"
Write-Host "   - Automatic container restart on failure"
Write-Host "   - Persistent data volume for SQLite database"
Write-Host "   - Health checks and monitoring"
Write-Host "   - Easy rollback to previous versions"
...
Write-Host "   2. For Docker issues: SSH to EC2 and run 'docker logs wedding-gift-api'"
Write-Host "   3. For setup issues: Make sure you ran setup-ec2-docker.sh on EC2"
Write-Host ""
...
Write-Host ".\deploy-docker.ps1 $Version"
